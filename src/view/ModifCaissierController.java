...
public class ModifCaissierController implements Initializable {
    ...
    Gestionnaire gest = gestDAO.find(AdminCaissierController.g);
    ...
    void handleOkButton(ActionEvent event) throws IOException, SQLException {
    	if(id.getText().equals("")) {
    		...
    		if (!Short.valueOf(id.getText()).equals((Short)AdminCaissierController.g) ) {
    	...
    	gestDAO.update(gest);
    	listCaissiers = gestDAO.findAllCaissier();
    	AdminCaissierController.data.setAll(listCaissiers);
    	main.getStageDelete1().close();
    }
